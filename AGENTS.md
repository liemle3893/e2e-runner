...
- Go 1.24+; run `make lint` (golangci-lint) before submitting.
- Exported identifiers carry doc comments; comments explain *why*, not *what*.
- Adapters implement the `Adapter` interface and reuse the helpers in
  `internal/adapter/adapter.go` — never reimplement timing or result wrapping.
- Assertions go through `internal/assertion` — never hand-roll operator logic in
  an adapter or the executor.
- Errors reach users through the constructors in `internal/tryve/errors.go`, so
  every failure carries a code and an actionable message.
- YAML test files are the test definition format. There is no TypeScript DSL and
  no TypeScript anywhere in the repository — if you find yourself adding a `.ts`
  file or a `package.json`, something has gone wrong.

## Testing

- Run `make test` before committing.
- Unit tests live beside the code (`internal/<pkg>/<file>_test.go`). Only Go
  integration tests belong under `tests/integration/`.
- New adapters require both unit tests and an E2E test at
  `tests/e2e/adapters/TC-<NAME>-001.test.yaml`.
- A test that asserts on behaviour must be able to fail for the right reason.
  When fixing a bug where an assertion was silently skipped, first write the test
  that reproduces the false pass.

## PR Guidelines

- Complete every item in the adapter checklist; no partial adapter implementations.
- Documentation must be in sync with code changes — see the Documentation Sync Rule.
- Never document a feature that is not implemented. Test authors write against the
  docs, and an unimplemented-but-documented field means their assertions silently
  do nothing.
